...
\begin{cadre}{Cosmogonie du système solaire}
  ...
  \begin{enumerate}[label=\upshape\alph*)]
    ...
    \item On note \(\vv a\) le champ (massique) des forces d'inerties d'entraînement dans le référentiel lié au fluide, et \(U\) le potentiel dont dérive \(\vv a\). Calculer le laplacien \(\Delta U\).
...
\begin{cadre}{Moment magnétique d'une sphère chargée en surface et en rotation}
  ...
  \begin{tikzpicture}[scale=2.5]
    \draw[->, >=stealth] (-0.5,0) -- (3,0) node[below]{\(z\)};
    \node at (2,0) [name=m] {\(\bullet\)};
    \node at (2,0) [below]{\(M\)};
    \draw[very thick] (0,-0.7) .. node[pos=0.3, name=a]{}  controls (0.2,-0.7) and (0.2,0.7) .. (0,0.7) node[name=b]{};
    \draw[very thick] (0,-0.7) .. node[pos=0.7, sloped, scale=0.7]{\(\blacktriangleleft\)} node[pos=0.7, left]{\(I\)} controls (-0.2,-0.7) and (-0.2,0.7) .. (0,0.7);
    \draw[->, >=latex] (0,0) node{\(\bullet\)} node[above]{\(O\)} -- node[midway, left]{\(R\)} (a.center);
    \draw (b.center) -- (m.center);
    \draw (1.5,0) arc(180:161:0.5) node[midway, left]{\(\alpha\)};
    \draw[ultra thick, ->, >=latex] (m.center) -- node[midway, above]{\(\vv B(M)\)} ++(0.4,0);
    \node at (5,0) {\(\vv B(M) = \dfrac{\mu_{0}I}{2R}\sin^{3}(\alpha)\uz\)};
...
\begin{cadre}{Équilibre d'un cadre rectangulaire dans un champs magnétique}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.6][h]
            ...
            \draw[->, >=stealth] (0,0,-4.3) -- (0,0,5) node[below]{\(x\)};
            \draw[->, >=stealth] (0,0,0) -- (0,3.8,0) node[right]{\(z\)};
            \draw[->, >=stealth] (0,0,0) -- (7,0,0) node[below]{\(y\)};
            \draw[ultra thick, <-, >=latex] (0,2,1.8) -- node[midway, left]{\(\vv g\)} ++ (0,2.5,0);
            \draw[ultra thick, ->, >=latex] (0,0,0) -- (0,2.5,0) node[right]{\(\vv B\)};
            \draw[ultra thick] (0,0,-4) -- (0,0,4) node[pos=0.75, sloped]{\(\blacktriangleleft\)};
            \draw[ultra thick] (0,0,-4) -- (5.5,3.3,-4) node[pos=0.3, sloped]{\(\blacktriangleleft\)} node[pos=0.3, above]{\(I\)};
            \draw[ultra thick] (0,0,4) -- (5.5,3.3,4) node[pos=0.75, sloped]{\(\blacktriangleright\)} node[pos=0.4, below]{\(2b\)};
            \draw[ultra thick] (5.5,3.3,-4) -- (5.5,3.3,4) node[pos=0.75, sloped]{\(\blacktriangleright\)};
            \draw[thick, dashed] (0,0,0) -- (5.5,3.3,0) node[right]{\(2a\)};
            \draw[thick, ->, >=latex] (3.3,0,0) arc(0:30.5:3.3) node[midway, right]{\(\theta\)};
        \end{tikzpicture}
    \end{center}
